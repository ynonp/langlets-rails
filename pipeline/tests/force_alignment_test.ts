...
Deno.test("fails without lyric lines to align", async () => {
  const { ctx, store } = makeCtx({ alignLyrics: stubAlign([]).align });

  await assertRejects(() => forceAlignment(ctx), Error, "extract_lyrics");
