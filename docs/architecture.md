...
`posthog.secret_key` (or `POSTHOG_SECRET_KEY`) for authenticated server features.
It uses the US ingest host unless `POSTHOG_HOST` is set. A missing project key disables event
...
native Library, API course enumeration, and sitemap
...
- **WordOrderActivity**: Sentence-building practice whose answer row declares the
  ...
  selected by the translation-language subdomain. Its check action, like its
...
The model reports only the three input problems a person can correct — a missing sentence, selection, or translation — as stable error codes. The controller translates those codes from `app.vocabulary_entries.new.errors`; malformed token ranges, missing language context, and record failures all use the localized generic save error instead of exposing internal diagnostics.
...
- **Home combines account progress with a Library preview** (`App::HomeController#index`). It has no YouTube/TikTok paste control — that flow belongs to the Create tab. Home loads the user's readable, published Enrollments, calculates completion in grouped queries, and keeps only unfinished courses for the progress surfaces. The two most recently practiced unfinished courses remain under the muted **"Continue"** heading. A separate **"Latest imports"** 2×2 grid reuses the complete mobile Library visibility scope, orders by Course creation time, and shows its newest four items whether or not the user has enrolled in them. Its **"View all"** link uses the native tab-selection bridge to switch to Library instead of pushing Library onto Home's navigator. Personal playlists follow when present. The started-videos screen still uses non-null `last_practiced_at` as its canonical started signal and includes completed courses.
  Latest-import cards reuse the compact Library card and retain its localized
...
The mobile Library's published items are ordered by `courses.created_at DESC`
(with ChannelItem id as a stable tie-breaker), so All and language-filtered
views put the most recently created courses first. Home's Latest imports shelf
uses the same scope and order before applying its four-item limit.
