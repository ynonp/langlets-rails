...
class TokenTranslationBlockParserTest < ActiveSupport::TestCase
  setup do
    @phrase = build_translated_phrase(text_l1: "Pensé que era un buen momento", text_l2: "I thought it was a good time", id: 1)
    @phrase2 = build_translated_phrase(text_l1: "هاي قصص عن الناس اللي", text_l2: "These are stories about people who", id: 2)
...
هاي قصص عن الناس [اللي] => These are stories about people [who]
  ...
  test "parses Arabic RTL text with multi-word mappings" do
    translations = @phrase2.add_tokens_from(@block2).translated_phrase_tokens
  ...
  test "parse block in Arabic" do
    ...
    p = build_translated_phrase(text_l1: "كان ييجي لعنده زباين من كل الأنحاء،", text_l2: "Customers would come to him from all over,")
    p.send(:process_line, line)

    assert p.translated_phrase_tokens.all? { |t| t.valid? }
  end

  test "spanish line with question mark" do
    p = build_translated_phrase("text_l1"=>"¿Quién es?", "text_l2"=>"Who is it?")
    block = <<~END
    [¿Quién] es? => [Who] is it?
    ¿Quién [es?] => Who [is it?]
    END
    p.add_tokens_from(block)
    assert_equal 2, p.translated_phrase_tokens.size
  end

  test "multiple translations with overlapping tokens" do
    p = build_translated_phrase(text_l1: "לא נגמר לנו היום", text_l2: "لم ينتهِ يومنا بعد", id: 999888)
    block = <<~END
    ...
    assert_equal 3, p.translated_phrase_tokens.size
    # The order depends on how they're stored after deduplication
    assert_equal 0, p.translated_phrase_tokens[0].l1_start_index
    assert_equal 15, p.translated_phrase_tokens[0].l1_end_index

    assert_equal 0, p.translated_phrase_tokens[1].l1_start_index
    assert_equal 6, p.translated_phrase_tokens[1].l1_end_index

    assert_equal 8, p.translated_phrase_tokens[2].l1_start_index
    assert_equal 15, p.translated_phrase_tokens[2].l1_end_index
  end

  test "parse line from flowers" do
    p = build_translated_phrase(text_l1: "'til we weren't", text_l2: "עד שכבר לא")
    p.send(:process_line, "['til] we weren't => [עד שכבר] לא")
    token = p.translated_phrase_tokens.first
  ...
  test "ignores comments and empty lines" do
    ...
    translations = @phrase.add_tokens_from(block_with_comment).translated_phrase_tokens
    assert_equal 5, translations.length
  end

  test "handles single word tokens" do
    simple_block = "Pensé que [era] un buen momento => I thought [it was] a good time"
    translations = @phrase.add_tokens_from(simple_block).translated_phrase_tokens
  ...
  test "skips invalid lines" do
    ...
    translations = @phrase.add_tokens_from(invalid_block).translated_phrase_tokens
    assert_equal 1, translations.length
  end

  test "returns empty for blank input" do
    assert_equal [], @phrase.add_tokens_from("").translated_phrase_tokens
    assert_equal [], @phrase.add_tokens_from(nil).translated_phrase_tokens
  end

  test "handles partial l1 sentence with missing prefix" do
    phrase = build_translated_phrase(
