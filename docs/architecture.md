...
The 2.x app pages render only the floating Add button (except Home, which swaps it for an inline dashed "bring your own" card, `app/views/app/home/_import_card.html.erb`); the native controller owns the tab bar and the `tab-badge` bridge mirrors the active-import count onto the Queue item. The native web views extend beneath `UITabBar`, so every scrolling app screen uses `app-scroll-pad` to reserve the full tab-bar height plus the bottom safe-area inset; the inset by itself only clears the home indicator. The server gates `/app` screens on the 2.x user agent so already-shipped 1.x builds remain on the regular web UI. Tab-root paths deliberately have no `replace_root` path-configuration rule because cross-tab routing is native; modal routes retain their existing rules.
...
- **Native tab-bar clearance**: Regular course and playlist pages mark 2.x native requests with `data-native-tabs`; the add-to-playlist and delete-playlist sheets use that state to reserve the iOS tab bar height plus the bottom safe-area inset and reduce their maximum height accordingly.
