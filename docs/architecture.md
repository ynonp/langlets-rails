...
- MatchTokensActivity uses the same frameless header (inline `0 / X matched` counter + progress bar + small instruction line) and a 2-column grid that pairs an L1 cell with its L2 translation. Both columns share the same center-aligned cell styling (matching `justify-center text-center` on the box, `text-center` on the word span) so a row reads as a balanced pair regardless of which language is LTR or RTL. The L1 and L2 columns are shuffled **independently** server-side so the L1 and L2 cells of the same pair are not in the same row (cells still match each other via the shared `data-token-id`). Tapping an L1 cell always plays the L1 audio for that pair (first tap, second tap, or switch); tapping an L2 cell is silent. Correct pairs flash green, then stay in place dimmed (lower opacity + grayscale + emerald ✓) instead of being hidden or replaced, mirroring the AudioToTranslation pattern. The grid is split into pages of four pairs (eight cells), and the next page is revealed only once every pair on the current page is matched.
