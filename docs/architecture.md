...
- The desktop Queue opts out of the application's root View Transition. The
  global transition begins the incoming document with a horizontal offset,
  which makes this standalone status page visibly slide sideways on desktop.
  `layouts/_head` supports the `disable_view_transition` content slot for this
  narrow exception; Turbo then performs an ordinary render both into and out of
  Queue. The native Create screen is unaffected.
