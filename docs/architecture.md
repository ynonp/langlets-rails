...
- **Empty Home is instructional, not a catalog.** If the account has no readable, published unfinished course (including an account whose courses are all complete), Home renders the localized text box beginning **"No langlets yet"** and explaining that sharing a Spanish, French, Hebrew, Arabic, or German video to Langlets creates language practice. It renders no global course cards and no first-run Create link. A course displayed in the just-imported hero still counts as unfinished, so the empty box does not appear beneath it.
