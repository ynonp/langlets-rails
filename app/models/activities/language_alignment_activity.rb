      ...
      
      {
        **video_params,
        ...
        phrases: preloaded_phrases.map do |p|
          {
            text: current_script ? p.text_l1.to_s(current_script) : p.text_l1.to_s,
            translation: p.text_l2.to_s,
            tokens: p.token_translations.filter_map do |t|
              {
                l1_start_index: current_script ? t.l1_characters_range(current_script).begin : t.l1_characters_range.begin,
                l1_end_index: current_script ? t.l1_characters_range(current_script).end - 1 : t.l1_characters_range.end - 1,
